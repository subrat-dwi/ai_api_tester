...
export function ResultCard({ attack, index, onClick }: ResultCardProps) {
  return (
    <motion.button
      ...
      className="group relative flex h-[132px] w-full flex-col justify-between overflow-hidden rounded-2xl border border-emerald-400/30 bg-emerald-950/40 px-4 py-4 text-left shadow-glowGreen backdrop-blur-xl"
    >
      ...
      <p className="relative max-h-[4.5rem] overflow-hidden font-mono text-xs leading-5 text-emerald-50/80">
        {attack.reason}
      </p>
    </motion.button>
  );
